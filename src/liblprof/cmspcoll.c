/* $Id: cmspcoll.c,v 1.5 2006/04/14 20:50:15 gfuer Exp $ */
...
DWORD MaskOfDataSet(LCMSHANDLE hSheet)
{
        ...
        for (i=0; i < n; i++) {
                ...
                if (IS("STDEV_X") || IS("STDEV_Y") || IS("STDEV_Z"))
                                dwMask |= PATCH_HAS_STD_XYZ;
                else
                if (IS("STDEV_L") || IS("STDEV_A") || IS("STDEV_B"))
                                dwMask |= PATCH_HAS_STD_Lab;
                else
                if (IS("STDEV_R") || IS("STDEV_G") || IS("STDEV_B"))
                                dwMask |= PATCH_HAS_STD_RGB;
                else
                if (IS("POS_X") || IS("POS_Y"))
                                dwMask |= PATCH_HAS_POS;
                else
                if (IS("STDEV_DE"))
...
BOOL cmsxPCollLoadFromSheet(LPMEASUREMENT m, LCMSHANDLE hSheet)
{
    ...
    for (i = 0; i < m->nPatches; i++) {
        ...
        if (dwMask & PATCH_HAS_STD_Lab) {

	    if (cmsxIT8GetDataSetDbl(hSheet, Patch->Name, "STDEV_L", &Patch->StdLab.L) &&
		cmsxIT8GetDataSetDbl(hSheet, Patch->Name, "STDEV_A", &Patch->StdLab.a) &&
		cmsxIT8GetDataSetDbl(hSheet, Patch->Name, "STDEV_B", &Patch->StdLab.b))

                                   Patch -> dwFlags |= PATCH_HAS_STD_Lab;
        }
        ...
        if (dwMask & PATCH_HAS_STD_XYZ) {

	    if (cmsxIT8GetDataSetDbl(hSheet, Patch->Name, "STDEV_X", &Patch->StdXYZ.X) &&
		cmsxIT8GetDataSetDbl(hSheet, Patch->Name, "STDEV_Y", &Patch->StdXYZ.Y) &&
		cmsxIT8GetDataSetDbl(hSheet, Patch->Name, "STDEV_Z", &Patch->StdXYZ.Z))

                                   Patch -> dwFlags |= PATCH_HAS_STD_XYZ;
        }
        ...
        if (dwMask & PATCH_HAS_STD_RGB) {

	    if (cmsxIT8GetDataSetDbl(hSheet, Patch->Name, "STDEV_R", &Patch->Stdev.RGB[0]) &&
		cmsxIT8GetDataSetDbl(hSheet, Patch->Name, "STDEV_G", &Patch->Stdev.RGB[1]) &&
		cmsxIT8GetDataSetDbl(hSheet, Patch->Name, "STDEV_B", &Patch->Stdev.RGB[2]))

                                Patch -> dwFlags |= PATCH_HAS_STD_RGB;
        }
        ...
        if (dwMask & PATCH_HAS_POS) {

	    if (cmsxIT8GetDataSetDbl(hSheet, Patch->Name, "POS_X", &Patch->pos_x) &&
		cmsxIT8GetDataSetDbl(hSheet, Patch->Name, "POS_Y", &Patch->pos_y))

                                Patch -> dwFlags |= PATCH_HAS_POS;
        }
    }
...
{			
	...
	if (dwMask & PATCH_HAS_STD_RGB) {
		cmsxIT8SetDataFormat(it8, nNumberOfFields++, "STDEV_R");
		cmsxIT8SetDataFormat(it8, nNumberOfFields++, "STDEV_G");
		cmsxIT8SetDataFormat(it8, nNumberOfFields++, "STDEV_B");
	}
	...
	for (i=0; i < m ->nPatches; i++) {
		if (m ->Allowed[i]) {
			...
			if (dwMask & PATCH_HAS_STD_RGB) {
				cmsxIT8SetDataSetDbl(it8, Patch->Name, "STDEV_R", Patch ->Stdev.RGB[0]);
				cmsxIT8SetDataSetDbl(it8, Patch->Name, "STDEV_G", Patch ->Stdev.RGB[1]);
				cmsxIT8SetDataSetDbl(it8, Patch->Name, "STDEV_B", Patch ->Stdev.RGB[2]);
			} 
		}
	}
...
BOOL cmsxPCollGetLighting(LPMEASUREMENT m, 
                          const char *MeasurementSheet,
                          DWORD dwNeededSamplesType)
{
    LCMSHANDLE hSheet;
    ...
    for (i = j = 0; i < m->nPatches; i++) {
    	if (tolower(m->Patches[i].Name[0]) == 'l' &&
	    tolower(m->Patches[i].Name[1]) == 'i' &&
	    tolower(m->Patches[i].Name[2]) == 'g' &&
	    tolower(m->Patches[i].Name[3]) == 'h' &&
	    tolower(m->Patches[i].Name[4]) == 't')
	{
	    if (i != j)
	    	m->Patches[j] = m->Patches[i];
	    j++;
	}
    }
...
int
cmsxPCollPatchesNearNeutralExt(LPMEASUREMENT m,
