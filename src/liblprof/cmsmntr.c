/* $Id: cmsmntr.c,v 1.9 2006/02/28 01:46:03 hvengel Exp $ */
...
BOOL cmsxMonitorProfilerDo(LPPROFILERDATA sys)
{
    ...
    AToB0 = BToA0 = NULL;
    if (!*sys -> hdr.MonOutputProfileFile)
        return FALSE;
    if (sys->hdr.ReferenceSheet[0] || sys->hdr.MonMeasurementSheet[0]) 
    {
        if (sys->hdr.cmsprintf) 
        {
            ...
            if (sys->hdr.ReferenceSheet[0])
                ...
                if (sys->hdr.MonMeasurementSheet[0])
                    sys->hdr.cmsprintf("Measurement sheet: %1", 1, 's', sys->hdr.MonMeasurementSheet);
            }
        if (!cmsxComputeMatrixShaper(sys -> hdr.ReferenceSheet,
                                     sys -> hdr.MonMeasurementSheet, 
                                     MEDIUM_TRANSMISSIVE,
        ...
        if (sys->hdr.cmsprintf) 
        {
            ...
            sys->hdr.cmsprintf("%1", 1, 's', Buffer);

            sys->hdr.cmsprintf("Primaries: R:%1, %2  G:%3, %4  B:%5, %6", 6, 'f',
    ...
    _cmsSaveProfile(sys->hdr.hProfile, sys->hdr.MonOutputProfileFile);
